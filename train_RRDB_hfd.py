...
if __name__ == '__main__':
    ...
    optimizerD_decay = optim.lr_scheduler.MultiStepLR(optimizerD_HF, decay_indices, gamma=0.5)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for data, target in train_bar:
            ...
            real_label_hf = torch.full((batch_size*3, 1), 1, dtype=z.dtype).cuda()
            fake_label_hf = torch.full((batch_size*3, 1), 0, dtype=z.dtype).cuda()

            netD_HF.zero_grad()
            real_out_hf = netD_HF(torch.cat([real_img_hf_1, real_img_hf_2, real_img_hf_3], dim=0))
            fake_out_hf = netD_HF(torch.cat([fake_img_1, fake_img_2, fake_img_3], dim=0))
            d_loss_real_hf = discriminator_criterion(real_out_hf-torch.mean(fake_out_hf), real_label_hf)
            d_loss_fake_hf = discriminator_criterion(fake_out_hf-torch.mean(real_out_hf), fake_label_hf)
            d_loss_hf = (d_loss_real_hf + d_loss_fake_hf)/2
            d_loss_hf.backward(retain_graph=True)
            optimizerD_HF.step()


            ############################
            # (3) Update G network: minimize 1-D(G(z)) + Perception Loss + Image Loss + TV Loss
            ###########################
            netG.zero_grad()
            g_loss = generator_criterion(fake_img, real_img, fake_out, real_out, real_label, fake_out_hf=fake_out_hf, real_out_hf=real_out_hf, real_label_hf=real_label_hf)
            g_loss.backward()
