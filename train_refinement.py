...
if __name__ == '__main__':
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        running_results = \
            {'batch_sizes': 0, 'd_loss': 0, 'g_loss': 0, 'r_loss': 0, 'd_score': 0, 'g_score': 0, 'r_score': 0}
        ...
        for data, target in train_bar:
            ...
            ############################
            # (3) Update R network: minimize L1 Loss between gen_img and gt
            ###########################
            netR.zero_grad()
            ...
            running_results['r_loss'] += refinement_loss.item() * batch_size
            running_results['d_score'] += real_out.item() * batch_size
            running_results['g_score'] += fake_out.item() * batch_size
            running_results['r_score'] += refine_out.item() * batch_size

            train_bar.set_description(desc=
                                      '[%d/%d] Loss_D: %.4f Loss_G: %.4f Loss_R: %.4f '
                                      'D(x): %.4f D(G(z)): %.4f R(G(z)): %.4f'
                                      % (
                epoch, NUM_EPOCHS,
                running_results['d_loss'] / running_results['batch_sizes'],
                running_results['g_loss'] / running_results['batch_sizes'],
                running_results['r_loss'] / running_results['batch_sizes'],
            ...
            if batches_done%100 == 0:
                ...
                refine_hr = utils.make_grid(refine_img, nrow=1, normalize=True)
        ...
        with torch.no_grad():
            ...
            for val_lr, val_hr_restore, val_hr in val_bar:
                ...
                batch_rmse = ((rsr - hr)**2).data.mean()
                valing_results['mse'] += batch_mse * batch_size
                valing_results['rmse'] += batch_rmse * batch_size
                batch_ssim = pytorch_ssim.ssim(sr, hr).item()
                batch_rssim = pytorch_ssim.ssim(rsr, hr).item()
                valing_results['ssims'] += batch_ssim * batch_size
                valing_results['rssims'] += batch_rssim * batch_size
                valing_results['psnr'] = \
                    10 * log10((hr.max()**2) / (valing_results['mse'] / valing_results['batch_sizes']))
                valing_results['rpsnr'] = \
                    10 * log10((hr.max() ** 2) / (valing_results['rmse'] / valing_results['batch_sizes']))
                valing_results['ssim'] = valing_results['ssims'] / valing_results['batch_sizes']
                valing_results['rssim'] = valing_results['rssims'] / valing_results['batch_sizes']
                val_bar.set_description(
                    desc='[converting LR images to SR images] PSNR: %.4f dB SSIM: %.4f RPSNR: %.4f dB RSSIM: %.4f' % (
                        valing_results['psnr'], valing_results['ssim'], valing_results['rpsnr'], valing_results['rssim']))
